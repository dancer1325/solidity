...
* ❌NOT apply | :ref:`solcjs <solcjs>`❌

## Basic Usage

* ``solc``
  * == Solidity CL compiler
  * one of the build targets | Solidity repository
  * produce VARIOUS outputs
    * [simple binaries / assembly over an abstract syntax tree (parse tree), estimations of gas usage]
    * 👀if you ONLY want to compile 1! file -> run ``solc --bin sourceFile.sol`` (== print the binary)👀
    * 👀if you want to output everything | SEPARATE files -> run ``solc -o outputDirectory --bin --ast-compact-json --asm sourceFile.sol``👀

## Optimizer Options -- `--optimize` --

* uses
  * compile | BEFORE deploying your contract
* ``solc --optimize --bin sourceFile.sol``
* assumptions
  * contract is called 200 times | its lifetime
    * == EACH `opcode` is executed == 200 times
* if you
  * want the initial contract deployment cheaper vs later function executions -> set ``--optimize-runs=1``
  * expect MANY transactions & NOT care for higher deployment cost & output size -> set ``--optimize-runs=higherNumber``
* effects |
  - binary search's size | function dispatch routine
  - way / constants (_Example:_ large numbers or strings) are stored
...
* CL compiler
  * AUTOMATICALLY read imported files -- from the -- filesystem
  * provide :ref:`path redirects <import-remapping>` -- via -- ``prefix=path``
...
* 👀if your contracts use :ref:`libraries <libraries>` -> bytecode contains
  * substrings of the form ``__$53aea86b7d70b31448b230b20ae141a537$__`` 👀
    * | Solidity <v0.5.0,
      * [DIFFERENT format](https://docs.soliditylang.org/en/v0.4.26/contracts.html#libraries)
    * 👀these substrings == placeholders -- for the -- actual library addresses👀
      * placeholder == fully qualified library name's keccak256 hash's hex encoding's 34 character prefix
  * lines | end /
    * 's form == ``// <placeholder> -> <fq library name>``
    * help identify the libraries / placeholders represent

* fully qualified library name
  * == `pathOfItsSourceFile:libraryName`
    * | Solidity v0.8.1,
      * ALSO valid `pathOfItsSourceFile=libraryName`
      * ``:``
        * as separator is deprecated
    * | Solidity v0.y.z
      * ``:``
        * will be removed

* ``solc``
  * can insert the library addresses | those points
    * == linker meaning
    * ways
      * ``solc --libraries "file.sol:Math=0x1234567890123456789012345678901234567890 file.sol:Heap=0xabCD567890123456789012345678901234567890"`` OR
      * store the string | file (1 library / line) + run ``solc --libraries fileName``
...
# Setting the EVM Version to Target -- `--evm-version` --

* `--evm-version`
  * == `solc --evm-version <VERSION> solidityFile.sol`
  * allows
    * | compile your contract code,
      * 👀specify the EVM version -- to -- compile for👀
  * uses
    * avoid particular features or behaviors
  * recommendations
    * if you run a private chain -> use matching EVM versions

* | :ref:`standard JSON interface <compiler-api>`,
  * use the ``"settings.evmVersion"``
...
* target EVM versions & compiler-relevant changes / introduced | EACH version
  * backward compatibility / EACH version
    * ❌NOT guaranteed❌

- ``homestead``
   - OLDEST version
   - ⚠️support deprecated⚠️
- ``tangerineWhistle``
...
- ``byzantium`` (*support deprecated*)
   - opcodes / AVAILABLE | assembly
     - ``returndatacopy``,
     - ``returndatasize``
     - ``staticcall``
       - uses
         - | call non-library view or pure functions / prevents the functions -- from -- modifying state | EVM level
           - == if you use invalid type conversions -> even applies
   - FROM function calls,
     - POSSIBLE to access dynamic data returned
   - ``revert`` opcode
     - == NOT waste gas
- ``constantinople``
   - opcodes / AVAILABLE | assembly
     ...
     - ``sar``
   - shifting operators
     - use shifting opcodes
       - -> need LESS gas
- ``petersburg``
   - 's behavior == compiler behaves -- as with -- constantinople
- ``istanbul``
   - opcodes / AVAILABLE | assembly
     - ``chainid``
     - ``selfbalance``
- ``berlin``
   - Gas costs -- for -- increased of
     - ``SLOAD``,
     - ``*CALL``,
     - ``BALANCE``,
     - ``EXT*``
     - ``SELFDESTRUCT``
   - compiler
     - assumes cold gas costs / such operations
   - relevant -- for --
     - gas estimation
     - optimizer
- ``london``
   - block's base fee
     - [EIP-3198](https://eips.ethereum.org/EIPS/eip-3198) & [EIP-1559](https://eips.ethereum.org/EIPS/eip-1559)
     - ways to access | inline assembly,
       - global ``block.basefee`` OR
       - ``basefee()``
- ``paris``
