...
* follow [semver](https://semver.org)
  * == versions are bumped -- based on -- SemVer & change's severity
  * 👀patch-level releases / major release 0 (i.e. 0.x.y) does NOT contain breaking changes 👀
    * == code / compiles with v0.x.y -> expected to compile with 0.x.z / z > y
  * Reason: 🧠works well -- with -- [version pragma](layout-of-source-files.md#version-pragma)🧠

* **nightly development builds**
  * enable
    * try out upcoming features
  * ⚠️NOT guarantee to be ALWAYS working⚠️
    * if bug is identified -> NOT used | production
  * create a release / ``prerelease`` specifier
